App.Utils = {
    translations: {
        'est': {
            ...
            'Yes': 'Jah',
            'Thank you': 'Aitäh',
            'You can use this code to watch the movie on any devices. A copy of the code has also been sent to your email': 'Võid kasutada seda koodi filmi vaatamiseks iga seadme pealt. Kood on ka saadetud sinu e-mail aadressile'
        },
        'en': {
            'eesti': 'Estonian',
            'english': 'English'
    ...
    translate: function(string) {
        var str = _.filter(App.Utils.translations[App.Settings.language], function(item, key) {
            ...
            deferreds.push($.ajax({
                ...
                success: function(response) {
                    ...
                    items.each(function(idx, item) {
                        var id = $(item).attr("id");
                        if ($(item).hasClass("helper") === true) ich.addHelper(id, $(item).html(), $(item).attr("data-args"));
                        else if ($(item).hasClass("partial") === true) ich.addPartial(id, $(item).html());
                        else ich.addTemplate(id, $(item).html());
                    });
                }
            }));
        })
...
App.Utils.Api = Backbone.Model.extend({
    ...
    doRequest: function(action, params, callback, silent, type) {
    ...
    post: function(action, params, callback, silent) {
        this.doRequest(action,params,callback,silent, "post");
    },
    call: function(action, params, callback, silent) {
        this.doRequest(action,params,callback,silent, "get");
    }
}),
App.Utils.Notification = Backbone.Model.extend({
    ...
    attach: function(model) {
        model.on("error", function(message, callback) {
            this.notify(message, "error", callback);
        }, this);
        model.on("notice", function(message, callback) {
            this.notify(message, "notice", callback);
        }, this);
        model.on("success", function(message, callback) {
            this.notify(message, "success", callback);
        }, this);
        model.on("flash", function(message, amount) {
            this.flash(message, amount);
        }, this);
    },
