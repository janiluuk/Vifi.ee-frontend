/*!
  ICanHazHandlebars.js -- by @jrgrafton, based on ICanHaz.js by @HenrikJoreteg
*/
...
(function(a){
;function b(){var c=this;c.VERSION="0.1";c.templates={};c.addTemplate=function(d,e){if(c[d]){throw"Invalid name: "+d+"."}if(c.templates[d]){throw'Template " + name + " exists'}c.templates[d]=Handlebars.compile(e);c[d]=function(h,g){h=h||{};var f=c.templates[d](h);return g?f:a(f)}};c.addPartial=function(d,e){if(Handlebars.partials[d]){throw'Partial " + name + " exists'}Handlebars.registerPartial(d,e)};c.addHelper=function(e,f,d){if(Handlebars.helpers[e]){throw'Helper " + name + " exists'}if(typeof f==="function"){Handlebars.registerHelper(e,f)}else{Handlebars.registerHelper(e,new Function(d,f))}};c.grabTemplates=function(){a('script[type="text/html"]').each(function(e,d){var f=a((typeof e==="number")?d:e),g=("".trim)?f.html().trim():a.trim(f.html());if(f.hasClass("partial")){c.addPartial(f.attr("id"),g)}else{if(f.hasClass("helper")){c.addHelper(f.attr("id"),g,f.attr("data-args"))}else{c.addTemplate(f.attr("id"),g)}}f.remove()})};c.clearAll=function(){for(var d in c.templates){delete c[d]}c.templates={};Handlebars.partials={}};c.refresh=function(){c.clearAll();c.grabTemplates()}}window.ich=new b();a(function(){ich.grabTemplates()})})(window.jQuery||window.Zepto);